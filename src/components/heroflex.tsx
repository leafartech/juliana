...
export default function Heroflex({ children, height, imagePath, width, alt, imgClassName, reverse }: heroflexProps) {
    return (
        <div className={`sm:grid sm:grid-cols-2 gap-4 sm:gap-0 ga flex items-center ${reverse ? 'flex-col-reverse' : 'flex-col'}`}>
            {!reverse && (
                <div className="w-full">
                    <Image
                        ...
                        height={height}
                        layout="responsive"
                        className={`w-full sm:max-w-[450px] ${imgClassName}`}
                    />
                </div>
            )}
            {children}
